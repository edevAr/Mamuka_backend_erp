...
public class SaleController {
     ...
     * Create a new sale
     */
     @PreAuthorize("hasAuthority('SALES_*') or hasRole('ADMIN')")
    @PostMapping
     ...
     */
    @GetMapping("/employee-store/{idEmployeeStore}")
    public ResponseEntity<List<SaleResponseDto>> getSalesByEmployeeStore(@PathVariable Long idEmployeeStore) {
        try {
            List<SaleResponseDto> sales = saleService.getSalesByEmployeeStore(idEmployeeStore);
            return ResponseEntity.ok(sales);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest().build();
        } catch (Exception e) {
    ...
    @PreAuthorize("hasAuthority('SALES_*') or hasRole('ADMIN')")
    @PostMapping("/report")
    public ResponseEntity<SalesReportResponseDto> generateSalesReport(@Valid @RequestBody SalesReportRequestDto request) {
        try {
            // Validate date range
            ...
            SalesReportResponseDto report = saleService.generateSalesReport(request);
    ...
    public ResponseEntity<?> generateSalesAnalysis(@Valid @RequestBody SalesAnalysisRequestDto request) {
        try {
            // Validate date range
            if (request.getStartDate() == null || request.getEndDate() == null) {
                return ResponseEntity.badRequest()
                        .body(new com.mamukas.erp.erpbackend.application.dtos.response.MessageResponseDto("startDate and endDate are required"));
            }
            
            if (request.getStartDate().isAfter(request.getEndDate())) {
                return ResponseEntity.badRequest()
                        .body(new com.mamukas.erp.erpbackend.application.dtos.response.MessageResponseDto("startDate cannot be after endDate"));
            }
            
            SalesAnalysisResponseDto report = saleService.generateSalesAnalysis(request);
            return ResponseEntity.ok(report);
        } catch (IllegalArgumentException e) {
            return ResponseEntity.badRequest()
                    .body(new com.mamukas.erp.erpbackend.application.dtos.response.MessageResponseDto(e.getMessage()));
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body(new com.mamukas.erp.erpbackend.application.dtos.response.MessageResponseDto("Internal server error: " + e.getMessage()));
        }
    }
}
